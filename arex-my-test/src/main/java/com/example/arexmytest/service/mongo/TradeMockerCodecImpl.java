...
public class TradeMockerCodecImpl implements Codec<TradeMocker> {
    private final ClassModel<TradeMocker> classModel;

    public TradeMockerCodecImpl(ClassModel<TradeMocker> classModel) {
        this.classModel = classModel;
    }

    @Override
    public TradeMocker decode(BsonReader reader, DecoderContext decoderContext) {
        TradeMocker instanceCreator = new TradeMocker();
        this.decodeProperties(reader, decoderContext, instanceCreator);
        return instanceCreator;
    }

    @Override
    public void encode(BsonWriter writer, TradeMocker value, EncoderContext encoderContext) {
    ...
    private boolean isRecordIdPropertyModel(PropertyModel<?> propertyModel) {
        return StringUtils.equals("recordId", propertyModel.getName());
    }

    @Override
    public Class<TradeMocker> getEncoderClass() {
        return TradeMocker.class;
    }

    private void encodeIdProperty(BsonWriter writer, TradeMocker instance, EncoderContext encoderContext, PropertyModel<String> idPropertyModel) {
        String idValue = getIdPropertyValue(instance);
        this.encodeValue(writer, encoderContext, idPropertyModel, idValue);
    }

    private String getIdPropertyValue(TradeMocker instance) {
    ...
    private <S> void encodeProperty(BsonWriter writer, TradeMocker instance, EncoderContext encoderContext, PropertyModel<S> propertyModel) {
        if (propertyModel != null && propertyModel.isReadable()) {
            S propertyValue = (S) propertyModel.getPropertyAccessor().get(instance);
    ...
    private void decodeProperties(BsonReader reader, DecoderContext decoderContext, TradeMocker instanceCreator) {
    ...
    private void decodePropertyModel(BsonReader reader, DecoderContext decoderContext, TradeMocker instance, String name) {
